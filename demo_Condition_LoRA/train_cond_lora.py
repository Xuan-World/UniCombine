...
from demo_Condition_LoRA.stylebooth_loader import StyleDataset,collate_fn
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_condition_lora",type=str,default="style",)
...
def main(args):
    ...
    transformer.add_adapter(transformer_lora_config,adapter_name=args.output_condition_lora)
    logger.info(f"Trainable lora: {args.output_condition_lora} is loaded successfully")

    # hook
    accelerator.register_save_state_pre_hook(functools.partial(save_model_hook,wanted_model=transformer,accelerator=accelerator,adapter_names=[args.output_condition_lora]))
    accelerator.register_load_state_pre_hook(functools.partial(load_model_hook,wanted_model=transformer,accelerator=accelerator,adapter_names=[args.output_condition_lora]))
    ...
    train_dataset = StyleDataset(csv_path="demo_Condition_LoRA/fixed_file.csv",root_dir="datasets/stylebooth_dataset", args = args)
    ...
    with preserve_requires_grad(transformer):
        transformer.set_adapters( [args.output_condition_lora] + ["schnell_assistant"])
    logger.info(f"Set Adapters:{[args.output_condition_lora] + ["schnell_assistant"]}")
